...
\begin{frame}{Data Quality \& Rules Engine}
  \begin{columns}[c]
    ...
    % Left Column: Key Validation Rules
    \begin{column}{0.6\textwidth}
      \begin{block}{Edge Validation (\texttt{quality.py})}
        Executes checks before writing to disk:
        \begin{itemize}
          \item \textbf{Structure \& Types}: Non-empty fields, valid JSON parse, schema matching.
          \item \textbf{Business Rules}: Valid ISO timestamps, allowed sources \& event types.
          \item \textbf{Deduplication}: Micro-batch de-duping and historical raw log lookup.
        \end{itemize}
      \end{block}

      \begin{block}{Routing Outcomes}
        \begin{itemize}
          \item \textbf{Valid Stream}: Parquet to \texttt{data/raw/}.
          \item \textbf{Rejected Stream}: Audit log to \texttt{data/rejects/}.
        \end{itemize}
      \end{block}
    \end{column}

    % Right Column: Quality Flow Graphic
    \begin{column}{0.40\textwidth}
      \centering
      \includegraphics[width=\linewidth]{validation_flow_slide7.jpg} % Save your cropped Slide 7 image here
    \end{column}
...
% -----------------------------------------------------------------
% Slide 9: Live Demo Walkthrough - Taran
% -----------------------------------------------------------------
...
\begin{frame}{Engineering Challenges}
  ...
  \begin{block}{1. Multi-Container Orchestration}
    ...
    \textbf{Solution}: Added explicit health checks and dependency ordering (\texttt{depends\_on}) in Docker Compose.
  \end{block}
...
\begin{column}{0.61\textwidth}\centering\includegraphics[width=\linewidth,height=5.2cm,keepaspectratio]{screenshot.png}
